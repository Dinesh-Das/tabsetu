...
function expectLocalDate(actual: Date | null, year: number, monthIndex: number, day: number, hours: number, minutes: number): void {
...
describe("nextMatchingDate", () => {
  ...
  it("returns null for weekly schedules without selected days", () => {
    const next = nextMatchingDate(schedule({ type: "weekly", daysOfWeek: [] }), new Date(2026, 4, 5, 10, 0));
