...
export default function SessionDetail({ session, onClose, addToast }: Props) {
  ...
  const [name, setName] = useState(session.name),
    [description, setDescription] = useState(session.description);
  const [note, setNote] = useState(session.note),
    [folderId, setFolderId] = useState(session.folderId ?? "");
  ...
  const handleAddCurrentTabs = async () => {
    ...
    addToast(
      "success",
      `Added ${addedCount} ${addedCount === 1 ? "open tab" : "open tabs"} to "${session.name}".`
    );
  };
    ...
    <aside className="session-detail-shell">
      <div className="session-detail-hero">
        <div className="session-detail-title-row">
          <div>
            <h2>{session.name}</h2>
            <p>
              Created {formatDateTime(session.createdAt)} - Opened{" "}
              {formatDateTime(session.lastOpenedAt)}
            </p>
          </div>
        ...
        <div className="session-detail-actions">
          ...
          <button
            className="btn btn-secondary"
            onClick={() => window.dispatchEvent(new Event("tabsetu:open-share-panel"))}
          >
            <Share2 size={15} />
            Share
          </button>
          <button
            className="btn btn-secondary"
            onClick={() => window.dispatchEvent(new Event("tabsetu:add-active-tab"))}
          >
            <Plus size={15} />
            Add active tab
          </button>
          <button className="btn btn-secondary" onClick={() => void handleAddCurrentTabs()}>
            <Plus size={15} />
            Add open tabs
          </button>
        </div>
      ...
      <div className="session-detail-scroll">
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setTagIds((current) =>
                          active
                            ? current.filter((existing) => existing !== tag.id)
                            : [...current, tag.id]
                        )
                      }
                      style={{
        ...
        <SessionSchedulePanel
          ...
          schedules={schedules}
          schedulesEnabled={settings.schedulesEnabled}
        ...
        <SessionTabList
          ...
          removeTabFromSession={removeTabFromSession}
          updateTabNote={updateTabNote}
          updateTabReminder={updateTabReminder}
          updateTabFolder={updateTabFolder}
          updateTabTags={updateTabTags}
